    ...
    <div className="h-full w-full relative flex items-center justify-center p-0 overflow-hidden bg-slate-950">
      {data.content.videoBg && (
          <div className="absolute inset-0 z-0 opacity-60">
             <video autoPlay loop muted playsInline className="w-full h-full object-cover brightness-50 contrast-125"><source src={data.content.videoBg} type="video/mp4" /></video>
      ...
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center max-w-7xl px-6 md:px-12 py-10">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            <div className="bg-white/10 backdrop-blur-2xl p-8 md:p-10 rounded-3xl shadow-2xl border border-white/20">
                ...
                <ul className="space-y-5 font-mono text-sm md:text-lg text-slate-100">{data.content.objectives?.map((obj: string, i: number) => <li key={i} className="flex items-start gap-4 leading-tight"><span className="text-amber-500 font-black">0{i+1}</span><span>{obj}</span></li>)}</ul>
            </div>
            <div className="bg-slate-900/90 backdrop-blur-2xl p-8 md:p-10 rounded-3xl shadow-2xl border border-amber-500/30">
                ...
                <div className="flex flex-col gap-5">{data.content.grammar?.map((g: string, i: number) => <div key={i} className="bg-slate-950/80 px-6 py-5 border-l-8 border-amber-600 text-slate-100 font-mono text-xs md:text-base rounded-r-2xl shadow-inner border border-white/5"><span className="text-amber-500 font-black block text-[10px] mb-1">STRAT-0{i+1}</span> {g}</div>)}</div>
            </div>
            <div className="bg-white/10 backdrop-blur-2xl p-8 md:p-10 rounded-3xl shadow-2xl border border-white/20 lg:col-span-1 md:col-span-2">
                ...
                <ul className="space-y-5 font-mono text-sm md:text-lg text-slate-200">{data.content.expectedOutcomes?.map((out: string, i: number) => <li key={i} className="flex items-center gap-4 italic"><span className="text-blue-400 font-bold">➤</span><span>{out}</span></li>)}</ul>
...
export const IceBreakerSlide: React.FC<{ data: SlideData; onNext?: () => void }> = ({ data }) => {
    return (
        <div className="h-full w-full bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
            <div className="absolute inset-0 opacity-20"><div className="absolute top-0 left-0 w-96 h-96 bg-blue-600 rounded-full blur-[150px] animate-pulse"></div><div className="absolute bottom-0 right-0 w-96 h-96 bg-amber-600 rounded-full blur-[150px] animate-pulse delay-700"></div></div>
            <div className="relative z-10 max-w-5xl w-full bg-white rounded-[3rem] overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.5)] flex flex-col md:flex-row border-[12px] border-slate-900">
                ...
                <div className="p-12 md:p-20 md:w-3/5 flex flex-col justify-center bg-slate-50">
                    <div className="mb-10 flex items-center gap-4"><span className="w-12 h-1 bg-amber-500 rounded-full"></span><span className="text-slate-400 font-mono font-black text-sm uppercase tracking-widest">Operational Inquiry</span></div>
                    ...
                    <div className="flex flex-col gap-4"><div className="text-slate-500 font-mono text-xs uppercase font-black tracking-widest flex items-center gap-3"><span className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></span> Awaiting verbal response from class...</div></div>
                </div>
            </div>
        </div>
    );
    ...
    <div key={data.id} className="h-full w-full flex flex-col md:flex-row bg-white overflow-hidden animate-in fade-in duration-500">
      <div className="flex-1 flex flex-col relative h-1/2 md:h-full overflow-y-auto border-r border-slate-200 custom-scrollbar">
          <div className="p-4 border-b border-slate-200 bg-slate-50 sticky top-0 z-20 flex justify-between items-center px-6 shadow-sm">
             <div><h2 className="text-2xl font-black font-mono text-slate-900 uppercase tracking-tighter">{data.title}</h2><p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mt-0.5">Find and click all Past Tense verbs in the text!</p></div>
             <div className={`transition-all duration-500 px-4 py-2 rounded-xl font-mono font-black shadow-lg flex flex-col items-center min-w-[120px] ${isComplete ? 'bg-green-600 scale-105' : 'bg-blue-600'}`}><span className="text-[10px] uppercase opacity-80">{isComplete ? 'Area Secured' : 'Past Actions'}</span><span className="text-xl flex items-center gap-2">{foundCount} / {totalVerbs} {isComplete && <span className="text-white animate-in zoom-in">✓</span>}</span></div>
          </div>
          <div className="p-6 md:p-16 flex-1 font-serif text-xl md:text-4xl leading-[1.8] text-slate-800 space-y-12">{paragraphs.map((para: string, idx: number) => (<div key={idx} className="relative pl-10 border-l-8 border-slate-100 hover:border-blue-600 transition-colors"><p><ReadingParser text={para} onVerbFound={() => setFoundCount(c => c + 1)} /></p></div>))}</div>
               ...
               {activeVocab && (
                   <div className="absolute bottom-full left-0 right-0 bg-white border-t-8 border-blue-600 p-8 shadow-[0_-20px_50px_rgba(0,0,0,0.1)] z-30 animate-in slide-in-from-bottom-5">
                       <div className="flex justify-between items-start max-w-3xl mx-auto">
                           <div><span className="text-blue-600 font-mono text-[10px] uppercase font-black tracking-widest bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Intel Definition</span><div className="font-black text-slate-900 text-3xl mt-3">{activeVocab.word}</div><div className="text-slate-700 text-xl italic mt-4 leading-relaxed font-serif bg-slate-50 p-4 rounded-xl border-l-4 border-slate-200">{activeVocab.definition}</div></div>
    ...
    return (
        <div className="h-full w-full bg-[#1a1a1a] flex flex-col items-center justify-center p-4 overflow-hidden relative">
            ...
            <div className="relative z-10 w-full max-w-7xl h-full flex flex-col overflow-hidden">
                <div className="text-center py-4 shrink-0"><div className="inline-block bg-amber-600 text-slate-950 px-4 py-1 rounded-full text-xs font-mono font-black uppercase mb-2 shadow-xl animate-pulse tracking-widest">SITREP: Data Scramble Error</div><h3 className="text-3xl md:text-5xl font-black text-amber-500 uppercase tracking-tighter drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] leading-none">{data.title}</h3></div>
                    ...
                    <div className="bg-slate-900/60 backdrop-blur-xl p-6 rounded-[2rem] border-4 border-slate-800 flex flex-col overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4 pb-10">
                            {shuffledItems.map((item) => {
                                ...
                                return (<button key={item.id} onClick={() => handleSelect(item.id)} disabled={isUsed} className={`w-full p-4 rounded-xl text-left font-serif transition-all transform shadow-lg relative ${isUsed ? 'opacity-0 scale-50 grayscale pointer-events-none translate-x-full absolute' : 'bg-[#fffcf0] border-l-[10px] border-amber-600 text-slate-800 hover:-translate-y-1 hover:shadow-amber-500/20 group active:scale-95'}`} style={{ transform: `rotate(${isUsed ? 0 : rotation}deg)` }}><div className="text-[9px] text-amber-700 font-mono font-black mb-1 opacity-60 uppercase tracking-widest border-b border-amber-900/10 pb-1">Archive ID: #{item.id.toString().padStart(3, '0')}</div><div className="text-base md:text-xl font-black leading-tight">{item.parts.join(' ')}</div></button>);
                            })}
                        </div>
                    </div>
                    <div className="bg-slate-950/90 p-8 rounded-[2rem] border-4 border-slate-900 flex flex-col shadow-inner overflow-hidden">
                        <div className="mb-4 flex justify-between items-end border-b-2 border-slate-800 pb-2 shrink-0"><p className="text-amber-500 font-mono text-[10px] uppercase tracking-widest font-black">Timeline Recovery Status:</p><p className="text-slate-600 font-mono text-xs">{selectedIds.length} / {data.content.items.length}</p></div>
                        <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-2 pb-10">
                            {Array.from({ length: data.content.items.length }).map((_, i) => {
                                ...
                                return (<div key={i} className={`min-h-[60px] md:min-h-[70px] rounded-xl border flex items-center px-6 transition-all duration-500 group ${filledItem ? 'bg-green-950/20 border-green-500 text-green-100 shadow-[0_0_20px_rgba(34,197,94,0.1)]' : 'bg-slate-900/30 border-slate-800 border-dashed text-slate-600'}`}><span className={`text-[10px] font-mono font-black mr-4 transition-colors ${filledItem ? 'text-green-500' : 'opacity-20'}`}>STEP 0{i+1}</span><span className={`text-base md:text-xl font-serif italic ${filledItem ? 'opacity-100' : 'opacity-30'}`}>{filledItem ? filledItem.parts.join(' ') : '— ACCESSING INTEL —'}</span>{filledItem && <span className="ml-auto text-green-500 text-xl animate-in zoom-in">✓</span>}</div>);
                            })}
                        </div>
    ...
    return (
        <div className="h-full flex flex-col items-center p-2 bg-slate-900 overflow-y-auto pt-4 pb-12 custom-scrollbar">
            <div className="max-w-7xl w-full flex flex-col gap-6">
                <div className="text-center border-b-2 border-slate-800 pb-4 relative"><div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-0.5 rounded-full text-[10px] font-mono font-black tracking-widest animate-pulse">CLASSIFIED DATABASE</div><h2 className="text-3xl md:text-5xl font-mono font-black text-white uppercase tracking-tighter drop-shadow-sm">{data.title}</h2><p className="text-blue-500 font-mono font-black tracking-[0.3em] uppercase mt-2 text-xs md:text-sm">Click cards to decode Past Simple data</p></div>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {data.content.verbs.map((item: VerbChallengeItem, idx: number) => {
                        const isRevealed = revealedIds.has(idx);
                        return (<button key={idx} onClick={() => toggleReveal(idx)} className={`h-32 md:h-40 perspective-1000 group relative transition-all duration-700 w-full active:scale-95`}><div className={`relative w-full h-full transition-all duration-700 preserve-3d ${isRevealed ? 'rotate-y-180 shadow-[0_0_20px_rgba(59,130,246,0.3)]' : ''}`}><div className={`absolute inset-0 backface-hidden flex flex-col items-center justify-center p-4 bg-slate-800 border-2 border-slate-700 rounded-2xl shadow-xl`}><div className="text-[9px] text-slate-500 font-mono uppercase mb-2 tracking-widest">Base Proto</div><div className="text-xl md:text-2xl font-mono font-black text-slate-300 uppercase">{item.base}</div><div className="mt-4 text-[8px] text-blue-500 font-mono animate-pulse uppercase tracking-widest">[ DECODE ]</div></div><div className={`absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-center p-4 bg-blue-900 border-2 border-blue-500 rounded-2xl shadow-inner`}><div className="text-[9px] text-blue-300 font-mono uppercase mb-1 tracking-widest font-black opacity-80">{getRule(item)}</div><div className="text-2xl md:text-3xl font-mono font-black text-white uppercase drop-shadow-[0_0_10px_rgba(255,255,255,0.4)]">{item.past}</div><div className="mt-3 w-8 h-0.5 bg-white/20 rounded-full"></div></div></div></button>);
                    })}
                </div>
            </div>
...
// --- Grammar Recap (GÜNCELLENDİ: İnteraktif Cümle Kurucu Eklendi) ---
export const GrammarRecapSlide: React.FC<{ data: SlideData }> = ({ data }) => {
    // Akıllı Cümle Kurucu Havuzu
    const pools = {
        ...
        verb: [
            ...
            { v: "swam", transitive: false }, // Intransitive (Nesne almaz)
        ...
        <div className="h-full flex flex-col items-center p-4 bg-slate-100 overflow-y-auto pt-10 pb-20 custom-scrollbar">
             <div className="max-w-6xl w-full">
                ...
                <div className="bg-white border-4 border-blue-700 rounded-3xl p-8 shadow-2xl mb-12 transform hover:scale-[1.01] transition-all">
                    <h3 className="font-black text-blue-900 text-xl mb-6 flex items-center gap-4 uppercase tracking-tighter"><span className="w-5 h-5 bg-blue-600 rounded-full shadow-[0_0_15px_rgba(37,99,235,1)]"></span> TACTICAL SENTENCE CONSTRUCTOR (TAP TO BUILD)</h3>
                    
                    {/* Formül Butonları */}
                    <div className="bg-slate-950 p-6 rounded-t-2xl font-mono flex flex-wrap justify-center items-center gap-3 border-b-2 border-white/10">
                        <button onClick={() => updatePart('s')} className="bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded font-black text-white transition-all active:scale-90">SUBJECT</button>
                        <span className="text-white">+</span>
                        <button onClick={() => updatePart('v')} className="bg-green-600 hover:bg-green-500 px-4 py-2 rounded font-black text-white transition-all active:scale-90">VERB+ed</button>
                        <span className="text-white">+</span>
                        <button onClick={() => updatePart('o')} className={`bg-amber-600 hover:bg-amber-500 px-4 py-2 rounded font-black text-white transition-all active:scale-90 ${!isTransitive ? 'opacity-20 line-through grayscale cursor-not-allowed' : ''}`}>OBJECT</button>
                        <span className="text-white">+</span>
                        <button onClick={() => updatePart('p')} className="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded font-black text-white transition-all active:scale-90">PLACE</button>
                        <span className="text-white">+</span>
                        <button onClick={() => updatePart('t')} className="bg-red-600 hover:bg-red-500 px-4 py-2 rounded font-black text-white transition-all active:scale-90">TIME</button>
                    </div>

                    {/* Dinamik Cümle Göstergesi */}
                    <div className="bg-slate-900 p-10 rounded-b-2xl shadow-inner min-h-[120px] flex flex-wrap items-center justify-center gap-x-4 gap-y-4 text-center">
                        <span className="text-blue-400 font-black text-2xl md:text-3xl font-mono animate-in slide-in-from-left-2">{sentence.s}</span>
                        <span className="text-green-400 font-black text-2xl md:text-3xl font-mono animate-in slide-in-from-bottom-2">{sentence.v}</span>
                        {isTransitive && <span className="text-amber-400 font-black text-2xl md:text-3xl font-mono animate-in zoom-in">{sentence.o}</span>}
                        <span className="text-purple-400 font-black text-2xl md:text-3xl font-mono animate-in slide-in-from-right-2">{sentence.p}</span>
                        <span className="text-red-400 font-black text-2xl md:text-3xl font-mono animate-in fade-in">{sentence.t}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                    <div className="bg-slate-900 text-white rounded-3xl p-10 shadow-xl border-t-8 border-red-500">
                        <h3 className="font-black text-red-500 text-2xl mb-6 uppercase tracking-tighter">TIME ADVERBS</h3>
                        <div className="grid grid-cols-2 gap-4 font-mono text-base md:text-lg"><div>🚩 Yesterday</div><div>🚩 Two days ago</div><div>🚩 Last night</div><div>🚩 Last week</div><div>🚩 Last month</div><div>🚩 In 2023</div></div>
                    </div>
                    <div className="bg-white border-4 border-slate-200 rounded-3xl p-10 shadow-xl hover:border-red-500 transition-all">
                        ...
                        <div className="bg-slate-50 p-6 rounded-2xl mb-6 font-mono text-center text-xl text-red-900 border-2 border-slate-100 shadow-inner uppercase">SUBJECT + <span className="text-red-700 font-black">didn't</span> + BASE VERB</div>
                        <ul className="space-y-3 text-slate-700 text-lg font-serif italic leading-relaxed"><li>✅ I <span className="text-red-700 font-black underline">didn't clean</span> the dorm.</li><li>✅ He <span className="text-red-700 font-black underline">didn't study</span> yesterday.</li></ul>
                    </div>
                </div>
             </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {data.content.scenarios.map((scenario: any) => (
                        <div key={scenario.id} className={`p-5 rounded-xl border-2 transition-all shadow-xl flex flex-col justify-between ${results[scenario.id] === true ? 'bg-green-950/40 border-green-500' : results[scenario.id] === false ? 'bg-red-950/40 border-red-500 animate-shake' : 'bg-slate-950/30 border-slate-800 hover:border-blue-600'}`}>
                            ...
                            <div className="flex gap-2">{scenario.options.map((opt: string) => (<button key={opt} onClick={() => handleSelect(scenario.id, opt)} disabled={results[scenario.id] !== undefined} className={`flex-1 py-2 rounded text-xs font-mono font-black transition-all shadow-lg ${results[scenario.id] !== undefined ? (scenario.correct === opt ? 'bg-green-600' : (selections[scenario.id] === opt ? 'bg-red-600 opacity-50 scale-95' : 'bg-slate-800 opacity-20')) : 'bg-blue-700 hover:bg-blue-600 active:scale-95'}`}>{opt}</button>))}</div>
                        </div>
                    ))}
                </div>
             </div>
...
export const LegendDossierSlide: React.FC<{ data: SlideData }> = ({ data }) => {
    ...
    const handleSelect = (folderId: number, key: string, correct: string) => { if (results[folderId] !== undefined) return; setSelections(prev => ({ ...prev, [folderId]: key })); setResults(prev => ({ ...prev, [folderId]: key === correct })); };
    return (
        <div className="h-full w-full bg-[#0a0a0a] text-white p-6 overflow-y-auto custom-scrollbar">
            <div className="max-w-7xl mx-auto space-y-10">
                <div className="text-center"><h2 className="text-4xl md:text-6xl font-black font-mono text-amber-500 uppercase tracking-tighter">{data.title}</h2><p className="text-slate-500 font-mono mt-2 uppercase tracking-widest">{data.content.instruction}</p></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-20">{data.content.folders.map((folder: any) => {
                    const isAnswered = results[folder.id] !== undefined;
                    const isCorrect = results[folder.id] === true;
                    return (<div key={folder.id} className={`p-8 rounded-[2rem] border-4 transition-all ${isAnswered ? (isCorrect ? 'bg-green-950/20 border-green-600 shadow-[0_0_30px_rgba(22,163,74,0.3)]' : 'bg-red-950/20 border-red-600 animate-shake') : 'bg-slate-900 border-slate-800'}`}><div className="flex justify-between items-center mb-6"><span className="font-mono text-xs font-black text-slate-500 tracking-[0.3em] uppercase">{folder.label}</span>{isAnswered && (isCorrect ? <span className="text-green-500 font-black font-mono text-xs animate-pulse">✓ VERIFIED</span> : <span className="text-red-500 font-black font-mono text-xs">⚠ ERROR: CORRECT WAS {folder.correct}</span>)}</div><p className="text-xl md:text-2xl font-serif leading-relaxed mb-8 italic">{folder.text.split('______').map((part: string, i: number) => (<React.Fragment key={i}>{part}{i === 0 && <span className={`underline decoration-wavy px-2 font-black ${isAnswered ? (isCorrect ? 'text-green-400' : 'text-red-500 line-through') : 'text-amber-500'}`}>{selections[folder.id] || "______"}</span>}{i === 0 && isAnswered && !isCorrect && <span className="text-green-400 font-black ml-1 animate-in zoom-in">[{folder.correct}]</span>}</React.Fragment>))}</p><div className="grid grid-cols-3 gap-3">{shuffledKeysMap[folder.id].map((key: string) => (<button key={key} onClick={() => handleSelect(folder.id, key, folder.correct)} disabled={isAnswered} className={`py-3 rounded-xl font-mono font-black text-sm transition-all ${isAnswered ? (key === folder.correct ? 'bg-green-600 shadow-[0_0_15px_rgba(34,197,94,0.5)]' : (selections[folder.id] === key ? 'bg-red-600 opacity-50 scale-95' : 'bg-slate-800 opacity-20')) : 'bg-slate-800 hover:bg-slate-700 active:scale-95'}`}>{key}</button>))}</div></div>);
                })}</div>
            </div>
            <style>{`@keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-6px); } 75% { transform: translateX(6px); } } .animate-shake { animation: shake 0.4s ease-in-out; }`}</style>
        </div>
  ...
  return (
    <div className="h-full w-full bg-slate-900 flex items-center justify-center p-6 text-white overflow-hidden relative">
        ...
        <div className="max-w-4xl w-full relative z-10 bg-slate-950/80 backdrop-blur-3xl p-10 md:p-20 rounded-[4rem] border-8 border-slate-900 shadow-[0_50px_100px_rgba(0,0,0,0.8)]">
            ...
            <div className="space-y-6">{data.content.checklist.map((item: any, idx: number) => (<div key={idx} className="bg-slate-900/50 p-6 rounded-2xl border-2 border-slate-800 flex justify-between items-center group hover:border-green-600 transition-all"><span className="text-xl md:text-3xl font-mono font-black text-slate-300 group-hover:text-white">{item.text}</span><div className="flex items-center gap-4"><span className="text-green-500 font-mono font-black text-sm tracking-widest">{item.reflection}</span><div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(22,163,74,0.5)]">✓</div></div></div>))}</div>
        </div>
    </div>
  );
};
...
export const GrammarAnalysisSlide: React.FC<{ data: SlideData }> = () => null;
